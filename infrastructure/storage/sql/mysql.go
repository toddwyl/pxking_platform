...
import (
	...
	"github.com/go-eagle/eagle/infrastructure/container/group"
	"github.com/go-eagle/eagle/infrastructure/logger"
	xtime "github.com/go-eagle/eagle/infrastructure/time"
)
...
func NewMySQL(c *Config) (db *DB) {
	...
	db, err := Open(c)
	if err != nil {
		logger.Error("open mysql error(%v)", err)
		panic(err)
	}
	return
}
...
func slowLog(statement string, now time.Time) {
	...
	if du > _slowLogDuration {
		logger.Warn("%s slow log statement: %s time: %v", _family, statement, du)
	}
}
