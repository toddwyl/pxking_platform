...
func (c *Consumer) Handle(delivery <-chan amqp.Delivery, handler Handler, done chan error) {
	...
	for d := range delivery {
		logger.Infof("Consumer received a message: %s in queue: %s", d.Body, c.queueName)
		logger.Infof("got %dB delivery: [%v] %q", len(d.Body), d.DeliveryTag, d.Body)
	...
	logger.Infof("handle: async deliveries channel closed")
	done <- nil
}
...
func (c *Consumer) ReConnect() {
	for {
		...
		case err := <-c.connNotify:
			if err != nil {
				logger.Errorf("[rabbitmq] consumer - connection NotifyClose: %+v", err)
			}
		case err := <-c.channelNotify:
			if err != nil {
				logger.Errorf("[rabbitmq] consumer - channel NotifyClose: %+v", err)
			}
		case <-c.quit:
			return
		}
		...
		if !c.conn.IsClosed() {
			// 关闭 SubMsg message delivery
			if err := c.channel.Cancel(c.consumerTag, true); err != nil {
				logger.Errorf("[rabbitmq] consumer - channel cancel failed: %+v", err)
			}
			if err := c.conn.Close(); err != nil {
				logger.Errorf("[rabbitmq consumer] - conn cancel failed: %+v", err)
			}
		}

		// IMPORTANT: 必须清空 Notify，否则死连接不会释放
		for err := range c.channelNotify {
			logger.Errorf("[rabbitmq] consumer - channelNotify err: %+v", err)
		}
		for err := range c.connNotify {
			logger.Errorf("[rabbitmq] consumer - connNotify err: %+v", err)
		}
	...
	quit:
		for {
			...
			case <-c.quit:
				return
			default:
				logger.Infof("[rabbitmq] consumer - reconnect")

				if err := c.Run(); err != nil {
					logger.Errorf("[rabbitmq] consumer - failCheck: %+v", err)
