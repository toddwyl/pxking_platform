	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				c.chOpen = false
				logger.Error("send email queue err: %+v, retry after 10 second", err.(error))
		...
		for {
			...
			case m, ok := <-c.ch:
				if !ok {
					logger.Info("mail queue is close")
					c.chOpen = false
					return
				}
				if !open {
				...
				if err := mail.Send(s, m); err != nil {
					logger.Warnf("email send failed, %v", err)
				} else {
			...
			case <-time.After(time.Duration(c.Config.Keepalive) * time.Second):
				if open {
					if err := s.Close(); err != nil {
						logger.Warnf("can not close smtp conn, %v", err)
					}
				}
				open = false
			}
